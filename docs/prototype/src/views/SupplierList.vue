<template>
  <div class="page-body">
    <div class="filter-bar">
      ...
      <div class="form-group"><label class="form-label">状态</label><select class="form-select" v-model="fStatus"><option>全部</option><option>启用</option><option>停用</option></select></div>
    ...
    <table class="data-table">
      ...
      <tr v-for="s in pagedList" :key="s.code">
        <td style="color:var(--color-text-muted);font-size:12px">{{ s.code }}</td>
        <td>{{ s.name }}</td>
        <td>{{ s.contact }}</td>
        <td>{{ s.phone }}</td>
        <td style="font-size:13px">{{ s.address }}</td>
        <td><span class="badge" :class="s.active?'badge-active':'badge-disabled'">{{ s.active?'启用':'停用' }}</span></td>
        <td>{{ s.date }}</td>
        <td style="display:flex;flex-wrap:wrap;gap:4px;justify-content:center">
          ...
          <button class="btn btn-text btn-sm" :style="{color:s.active?'var(--color-danger)':'var(--color-primary)'}" @click="toggleStatus(s)">{{ s.active?'停用':'启用' }}</button>
        ...
        <div style="display:flex;flex-direction:column;gap:14px">
          ...
          <div v-if="formMode==='edit'">
            ...
            <div style="display:flex;border:1px solid var(--color-border);border-radius:8px;overflow:hidden;width:fit-content">
              <div :style="{padding:'8px 20px',cursor:'pointer',fontSize:'13px',background:form.active?'var(--color-primary)':'transparent',color:form.active?'white':'var(--color-text)',transition:'all 0.15s'}" @click="form.active=true">启用</div>
              <div :style="{padding:'8px 20px',cursor:'pointer',fontSize:'13px',background:!form.active?'var(--color-danger)':'transparent',color:!form.active?'white':'var(--color-text)',transition:'all 0.15s',borderLeft:'1px solid var(--color-border)'}" @click="form.active=false">停用</div>
            </div>
          </div>
...
<script>
import { getSuppliers, addSupplier, updateSupplier, deleteSupplier } from '../store/supplierStore.js'
import { isSupplierReferenced } from '../store/orderStore.js'
export default {
  emits: ['toast'],
  data: () => ({
    fKeyword:'', fStatus:'全部',
    showForm:false, showDelete:false, formMode:'add', deleteTarget:null,
    form:{code:'',name:'',contact:'',phone:'',address:'',active:true,remark:''},
    page:1, pageSize:5,
  }),
  computed:{
    ...
    filteredList(){
      ...
      return getSuppliers().filter(s=>{
        if(kw && !s.name.includes(kw) && !s.contact.includes(kw)) return false
        if(this.fStatus==='启用' && !s.active) return false
        if(this.fStatus==='停用' && s.active) return false
        return true
      })
    },
    pageNumbers(){
  ...
  methods:{
    query(){ this.page=1 },
    resetQuery(){ this.fKeyword=''; this.fStatus='全部'; this.page=1 },
    openAdd(){
      this.formMode='add'
      this.form={code:'SUP-'+String(getSuppliers().length+1).padStart(3,'0'),name:'',contact:'',phone:'',address:'',active:true,remark:''}
      this.showForm=true
    },
    openEdit(s){ this.formMode='edit'; this.form={...s}; this.showForm=true },
    saveForm(){
      if(!this.form.code||!this.form.name||!this.form.contact||!this.form.phone){ this.$emit('toast',{msg:'请填写必填字段（编号/名称/联系人/电话）',type:'warning'}); return }
      if(this.formMode==='add'){
        if(getSuppliers().some(s=>s.code===this.form.code)){ this.$emit('toast',{msg:'供应商编号已存在',type:'warning'}); return }
        addSupplier({...this.form,date:new Date().toISOString().slice(0,19).replace('T',' ')})
        this.$emit('toast',{msg:'新增供应商成功，状态为启用',type:'success'})
      }else{
        updateSupplier(this.form.code,{...this.form})
        this.$emit('toast',{msg:'编辑成功',type:'success'})
      }
      this.showForm=false
    },
    toggleStatus(s){
      updateSupplier(s.code,{active:!s.active})
      this.$emit('toast',{msg:'「'+s.name+'」已'+(!s.active?'启用':'停用'),type:'success'})
    },
    doDelete(s){
      if(isSupplierReferenced(s.code)){ this.$emit('toast',{msg:'该供应商已被订单引用，无法删除',type:'warning'}); return }
      this.deleteTarget=s; this.showDelete=true
    },
    confirmDelete(){
      if(this.deleteTarget){ deleteSupplier(this.deleteTarget.code); this.$emit('toast',{msg:'已删除供应商「'+this.deleteTarget.name+'」',type:'success'}) }
      this.showDelete=false; this.deleteTarget=null
    },
    doExport(){
      const data=this.filteredList.map(s=>[s.code,s.name,s.contact,s.phone,s.address,s.active?'启用':'停用',s.date])
      data.unshift(['供应商编号','名称','联系人','联系电话','地址','状态','创建时间'])
      ...
      xml.push('<Worksheet ss:Name="供应商列表"><Table>')
      ...
      a.href=url;a.download='供应商列表_'+new Date().toISOString().slice(0,10)+'.xls'
